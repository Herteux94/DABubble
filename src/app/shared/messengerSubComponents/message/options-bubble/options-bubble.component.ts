...
export class OptionsBubbleComponent implements OnInit {
  @Output() emojiSelected = new EventEmitter<{ emoji: string, userID: string }>(); // Emit an object containing emoji and userID
  ...
  deleteMessage() {
    if (this.messengerType === 'channels') {
      ...
      const updatedThreadLength = this.activeThreadService.activeThreadMessage.threadLength - 1;
  ...
  navigateToThread() {
    this.activeThreadService.loadActiveThreadAndMessages(this.message.messageID);
